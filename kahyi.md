...
스트림은 외부와 데이터를 주고받는 형태의 공간을 이야기합니다. 끝소리 ㅎ의 기본값이며, *nix의 경우 `/dev/null`, Windows의 경우 `NUL`로 연결됩니다.
...
- **ㅃ** + *끝소리 없음*: (나중 이동) 한 값을 뽑아 두 번 넣기.
- **ㅃ** + *__ㅇ__ 또는 __ㅎ__을 제외한 끝소리*: (나중 이동) 한 값을 뽑아 두 번 넣기. (새롬) [^1]
...
- **ㅉ** + *끝소리 없음*: (나중 이동) 한 값을 뽑아 0이면 뽑은 값을, 아니면 0부터 뽑은 값-1 사이의 아무 값을 넣기. (새롬)
- **ㅉ** + *__ㅇ__ 또는 __ㅎ__을 제외한 끝소리*: (나중 이동) 한 값을 뽑아 0이면 뽑은 값을, 아니면 0부터 뽑은 값-1 사이의 아무 값을 넣기. (새롬) [^1]
- **ㅊ** + *끝소리 없음*: (나중 이동) 한 값을 뽑아 0이 아니면 중성의 방향, 아니면 반대 방향으로 옮기기.
- **ㅊ** + *__ㅇ__ 또는 __ㅎ__을 제외한 끝소리*: (나중 이동) 한 값을 뽑아 0이 아니면 중성의 방향, 아니면 반대 방향으로 옮기기. [^1]
...
[^1]: 처리 도중 오류가 발생하면 끝소리 갈피로 *나중 이동*을 합니다. 끝소리 갈피가 없으면 일반 오류로 취급하고 방향을 반전합니다.
